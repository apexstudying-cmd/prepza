...
def test_reaction_metadata_is_recorded_without_plaintext_emoji():
    ...
    try:
        ...
        row = db.session.execute(text(
            'SELECT kind FROM chat_message_meta WHERE message_id = :id'
        ), {'id': message.id}).first()
...
def test_message_list_gets_read_receipt_counts_and_kind():
    ...
    try:
        ...
        ConversationParticipant.query.filter_by(
            conversation_id=chat.id, user_id=two.id
        ).update({'last_read_at': datetime.utcnow()})
        db.session.execute(text(
            'INSERT INTO chat_message_meta (message_id, conversation_id, kind) VALUES (:m, :c, :k)'
        ), {'m': message.id, 'c': chat.id, 'k': 'text'})
        db.session.commit()
        ...
        with app.test_request_context(f'/chats/{chat.id}/messages', method='GET'):
            with session_transaction(app, one.id):
                returned = _chat_metadata_after_request(response)
...
def test_reaction_events_do_not_create_unread_badges():
    ...
    try:
        message = Message(
            conversation_id=chat.id,
            sender_id=two.id,
            body='opaque-reaction-ciphertext',
            nonce='opaque-nonce',
        )
        db.session.add(message)
        db.session.flush()
        db.session.execute(text(
            'INSERT INTO chat_message_meta (message_id, conversation_id, kind) VALUES (:m, :c, :k)'
        ), {'m': message.id, 'c': chat.id, 'k': 'reaction'})
        db.session.commit()

        response = app.response_class(
            response=json.dumps({'chats': [{
                'id': chat.id, 'is_group': False, 'name': 'Test',
                'last_message': 'ciphertext', 'last_message_at': datetime.utcnow().isoformat(),
                'unread_count': 1,
            }]}),
            status=200,
            mimetype='application/json',
...
def session_transaction(flask_app, user_id):
    """Context manager helper for Flask's session in a test request."""
