...
def replace_explore(text: str) -> str:
    ...
    anchor = None
    if not match:
        # Explore was historically generated by this build transform. If a
        # later source patch removed the generated section, reconstruct it
        # from the canonical architecture below rather than allowing the
        # production build to fail with an undefined ExploreScreen.
        anchor = re.search(r"(?ms)^// ─── CREATE MODAL", text)
        if not anchor:
            raise SystemExit('Explore architecture patch: CREATE MODAL anchor not found')
...
'''
    ...
    return text[:anchor.start()] + new_screen + text[anchor.start():]
