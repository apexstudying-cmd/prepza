...
def _has_reusable_wrapper(source: str, material_type: str, public_name: str) -> bool:
    legacy_name = f"_legacy_{public_name}"
    return (
        f"def {legacy_name}(" in source
        and f'def {public_name}(' in source
        and f'material_type="{material_type}"' in source
        and "from ai_reusable_generation import generate_document_material" in source
    )


def _install_wrapper(source: str, material_type: str, public_name: str) -> str:
    if _has_reusable_wrapper(source, material_type, public_name):
        return source
