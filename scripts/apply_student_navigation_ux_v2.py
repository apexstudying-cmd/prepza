...
def main():
    ...
    quick_pattern = re.compile(r"            \{\[\s*\{ icon: '📤', label: 'Upload'.*?\]\}\.map\(\(t, i\) => \(", re.S)
    ...
    text, count = quick_pattern.subn(quick_replacement, text, count=1)
    if count == 0 and "label: 'My Study'" not in text:
        raise SystemExit('Home quick actions block not found')
...
function StudyMaterialsScreen({ setScreen, setActiveDocumentId }: { setScreen: (s: Screen) => void; setActiveDocumentId: (id: number | null) => void }) {
  ...
  useEffect(() => {
    ...
    api<{ documents: HomeDocument[] }>('/documents')
      .then(async res => {
        ...
        setDocuments(ready)
        const rows = await Promise.all(ready.slice(0, 20).map(async d => {
          try {
        ...
        if (!cancelled) setMaterialRows(rows.flat())
      })
  ...
  const openMaterial = (row:{documentId:number; type:string}) => {
    ...
    const destination: Screen = type === 'summary' ? 'summary' : type === 'flashcards' ? 'flashcards' : type === 'quiz' || type === 'practice_questions' ? 'quiz' : type === 'mind_map' || type === 'mindmap' ? 'mind-map' : type === 'podcast' ? 'podcast-player' : 'document-study'
  ...
  return <div style={{flex:1,display:'flex',flexDirection:'column',background:T.pageBg}}>
    <div style={{background:N.navy,padding:'0 18px 20px',flexShrink:0}}>
      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:10}}>
        ...
        <div><div style={{fontWeight:800,fontSize:18,color:'#fff'}}>My Study</div><div style={{fontSize:11,color:'rgba(255,255,255,0.45)',marginTop:2}}>Documents and the materials created from them</div></div>
      </div>
    ...
    <div style={{flex:1,overflowY:'auto',padding:16}} className="scrollbar-hide">
      {loading ? <GenerationLoading label="Loading your study library…"/> : error ? <GenerationError error={error}/> : tab==='documents' ? <>
        <div style={{fontSize:12,color:T.textMuted,marginBottom:12}}>Open a document to read, ask Ada about it, or create study materials.</div>
        ...
        <div style={{fontSize:12,color:T.textMuted,marginBottom:12}}>Every item here came from one of your documents. Tap one to replay it.</div>
        {materialRows.length===0 ? <EmptyState icon="✦" title="No study materials yet" sub="Open a document and create a summary, flashcards, practice questions, mind map, or podcast." action="Open My Documents" onAction={()=>setTab('documents')}/> : materialRows.map((row,i)=><button key={`${row.documentId}-${row.type}-${i}`} onClick={()=>openMaterial(row)} style={{width:'100%',textAlign:'left',background:T.card,border:`1px solid ${T.border}`,borderRadius:16,padding:15,marginBottom:10,cursor:'pointer',fontFamily:'Plus Jakarta Sans'}}><div style={{display:'flex',alignItems:'center',gap:12}}><div style={{width:44,height:44,borderRadius:12,background:`${N.navy}0D`,color:N.navy,display:'flex',alignItems:'center',justifyContent:'center',fontWeight:900,fontSize:18}}>{icon(row.type)}</div><div style={{flex:1,minWidth:0}}><div style={{fontWeight:800,fontSize:13,color:T.text}}>{label(row.type)}</div><div style={{fontSize:11,color:T.textMuted,marginTop:4}} className="line-clamp-1">From: {row.documentTitle}</div></div><div style={{color:T.textMuted}}>{Ic.chevR()}</div></div></button>)}
      </>}
...
'''
    ...
    text = replace_once(text, "  const isHome  = ['home','ai-tutor','opportunities','opportunity-detail','podcast-player','podcast-library','flashcards','quiz','summary','upload','processing','doc-ready','document-study','share-sheet','share-opp-form','edu-upload-form','notifications','library','mind-map'].includes(active)", "  const isHome  = ['home','ai-tutor','opportunities','opportunity-detail','podcast-player','podcast-library','flashcards','quiz','summary','upload','processing','doc-ready','document-study','study-materials','share-sheet','share-opp-form','edu-upload-form','notifications','library','mind-map'].includes(active)", 'Bottom navigation grouping')
