...
'''
    if anchor in s:
        ...
        current_emit = '    emit("chat:read", {"conversation_id": conversation_id, "user_id": user_id, "read_at": read_at}, to=room_for(conversation_id), include_self=False)'
        if current_emit not in s:
        ...
        app.logger.exception("Could not persist chat read receipts")
'''
        s = s.replace(current_emit, persistence + current_emit, 1)
