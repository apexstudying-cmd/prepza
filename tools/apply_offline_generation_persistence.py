...
def add_imports(text, path):
    old = "import { saveGeneratedMaterialOffline } from '../offline/generatedMaterials'\n"
    if old in text: return text.replace(old, IMPORT, 1)
    ...
    if anchor in text and "getGeneratedMaterialOffline" not in text: text = text.replace(anchor, IMPORT, 1)
    if "getGeneratedMaterialOffline" not in text: raise SystemExit(f'Offline generation: exact replay import missing in {path.name}')
    if "getSavedStudyHubOffline" not in text:
        text = text.replace(IMPORT, IMPORT + "import { getOfflineUserId } from '../offline/generatedMaterials'\nimport { getSavedStudyHubOffline } from '../offline/studyHubOffline'\n", 1)
    return text
...
"""
        ...
        if not m: raise SystemExit(f'Offline generation: fetch anchor missing in {path.name}')
        ...
        if 'getGeneratedMaterialOffline(path, requestBody)' not in text:
            text = text.replace('    const localGeneration = await getGeneratedMaterialOffline(path, null)', '    const exactGeneration = await getGeneratedMaterialOffline(path, requestBody)\n    if (exactGeneration !== null) return exactGeneration as T\n    const localGeneration = await getLatestGeneratedMaterialForPath(path)', 1)
    text = text.replace('    const localGeneration = await getLatestGeneratedMaterialForPath(path)\n    if (localGeneration !== null) return localGeneration as T', '    const exactGeneration = await getGeneratedMaterialOffline(path, requestBody)\n    if (exactGeneration !== null) return exactGeneration as T\n    const localGeneration = await getLatestGeneratedMaterialForPath(path)\n    if (localGeneration !== null) return localGeneration as T', 1)
    success = "  if (!res.ok) throw new GenerationApiError(body?.error || body?.message || `Request failed (${res.status})`, res.status)"
    if success not in text: success = "  if (!res.ok) throw new Error(body?.error || body?.message || `Request failed (${res.status})`)"
    if success not in text: raise SystemExit(f'Offline generation: API success anchor missing in {path.name}')
...
def patch_podcast(text, path):
    if 'PodcastGenerationScreen' not in text: return text
    if 'offlinePodcastAudio' not in text:
        anchor = "  const [playing, setPlaying] = useState(false)\n"
        if anchor not in text: raise SystemExit(f'Offline generation: podcast state anchor missing in {path.name}')
    ...
    if ready in text and 'cacheGeneratedAudioOffline(res.audio_url)' not in text: text = text.replace(ready, "          setAudioUrl(res.audio_url); setAudioDuration(res.duration_seconds || 0); void cacheGeneratedAudioOffline(res.audio_url); setPhase('ready'); return", 1)
    if '// Offline podcast audio source' not in text:
        anchor = "  useEffect(() => {\n    const audio = audioRef.current\n"
        if anchor not in text: raise SystemExit(f'Offline generation: podcast audio effect anchor missing in {path.name}')
...
"""
    ...
    if audio in text: text = text.replace(audio, '<audio ref={audioRef} src={offlinePodcastAudio || audioUrl} preload="metadata" />', 1)
    return text

def patch_flashcards(text, path):
    if 'FlashcardsGenerationScreen' not in text or '// Offline flashcards restore' in text: return text
    anchor = "  useEffect(() => {\n    if (activeDocumentId == null) return\n    Promise.all([generationApi<{ csrf_token: string }>('/me'), generationApi<{ title: string }>(`/documents/${activeDocumentId}`)])"
    if anchor not in text: raise SystemExit(f'Offline generation: flashcard document effect anchor missing in {path.name}')
...
def patch_summary(text, path):
    if 'SummaryGenerationScreen' not in text or '// Offline summary restore' in text: return text
    anchor = "  useEffect(() => {\n    if (activeDocumentId == null) return\n    Promise.all([generationApi<{ csrf_token: string }>('/me'), generationApi<{ title: string }>(`/documents/${activeDocumentId}`)])"
    positions = [m.start() for m in re.finditer(re.escape(anchor), text)]
    if not positions: raise SystemExit(f'Offline generation: summary document effect anchor missing in {path.name}')
...
def patch(path):
    ...
    text = add_imports(text, path); text = patch_api(text, path); text = patch_podcast(text, path); text = patch_flashcards(text, path); text = patch_summary(text, path)
    path.write_text(text, encoding='utf-8')

for file in FILES: patch(file)
