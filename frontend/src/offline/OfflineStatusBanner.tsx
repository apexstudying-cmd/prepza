...
export default function OfflineStatusBanner() {
  ...
  useEffect(() => {
    const onOffline = () => { setOnline(false); setSyncing(false); setVisible(true) }
    const onOnline = () => { setOnline(true); setSyncing(true); setVisible(true) }
    const onSyncing = () => { setSyncing(true); setVisible(true) }
    const onSynced = () => { setSyncing(false); setVisible(true); window.setTimeout(() => setVisible(false), 1200) }
  ...
  const label = !online ? 'Offline — saved study materials remain available' : syncing ? 'Syncing your study activity…' : 'All caught up'
  return (
