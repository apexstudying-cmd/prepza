          ...
          <div style={{overflowX:'auto',paddingBottom:3}}>
            <div style={{minWidth:390,position:'relative',paddingTop:17}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:`repeat(${weeks.length},10px)`,gridTemplateRows:'repeat(7,10px)',gridAutoFlow:'column',gap:3}}>
                {weeks.flatMap((w,wi)=>w.map((d,di)=><button key={d.date} type="button" aria-label={d.future ? 'Future' : `${d.date}: ${fmt(d.seconds)} studied`} title={d.future ? '' : `${d.date} · ${fmt(d.seconds)} studied`} disabled={d.future} onClick={() => !d.future && setSelectedDay({date:d.date,seconds:d.seconds})} style={{gridColumn:wi+1,gridRow:di+1,width:10,height:10,padding:0,borderRadius:2,border:d.future?'none':level(d.seconds)===0?'1px solid #E3E6EC':'none',background:d.future?'transparent':(['#F0F1F4',`${N.gold}35`,`${N.gold}70`,`${N.gold}A8`,N.gold][level(d.seconds)]),cursor:d.future?'default':'pointer'}}/>))}
              </div>
            </div>
          </div>
          {selectedDay && <div style={{marginTop:10,padding:'10px 12px',borderRadius:11,background:'#F8F9FC',display:'flex',justifyContent:'space-between',alignItems:'center'}}><div><div style={{fontSize:10,fontWeight:800,color:N.navy}}>{new Date(selectedDay.date+'T00:00:00').toLocaleDateString('en-KE',{weekday:'short',month:'short',day:'numeric'})}</div><div style={{fontSize:11,color:'#9299A9',marginTop:2}}>Study time</div></div><div style={{fontSize:14,fontWeight:800,color:N.gold}}>{fmt(selectedDay.seconds)}</div></div>}
          {selectedDay && <div style={{marginTop:10,padding:'10px 12px',borderRadius:12,background:'#F8F9FC',display:'flex',justifyContent:'space-between',alignItems:'center'}}><div><div style={{fontSize:10,color:'#9299A9'}}>SELECTED DAY</div><div style={{fontSize:12,fontWeight:800,color:N.navy,marginTop:2}}>{selectedDay.date}</div></div><div style={{fontSize:13,fontWeight:800,color:N.gold}}>{fmt(selectedDay.seconds)} studied</div></div>}
