...
function installConversationObserver() {
  ...
  window.fetch = async (input, init) => {
    ...
    if (match || detail) {
      ...
      if (!recentlyDismissed) { observedConversationId = id; observedAt = Date.now() }
    }
...
export default function WhatsAppChatExperience() {
  ...
  useEffect(() => {
    ...
    const sync = () => {
      ...
      if (!nextActive) { if (!cancelled) setActive(false); return }
      if (!cancelled) { setActive(true); if (conversationId !== id) setConversationId(id) }
    }
  ...
  useEffect(() => {
    const onStatus = (event: Event) => setRealtimeConnected(Boolean((event as CustomEvent<{ connected?: boolean }>).detail?.connected))
    window.addEventListener('prepza-realtime-status', onStatus)
  ...
  const send = async () => {
    ...
    setSending(true); setError(''); sendTypingRealtime(conversationId, false)
    try {
      const message = await api<Message>(`/chats/${conversationId}/messages`, { method: 'POST', body: JSON.stringify({ body }) })
      setMessages(current => [...current.filter(m => m.id !== message.id), message]); setInput('')
  ...
  const handleInput = (value: string) => {
    ...
    sendTypingRealtime(conversationId, value.trim().length > 0)
    if (typingTimer.current != null) window.clearTimeout(typingTimer.current)
    typingTimer.current = window.setTimeout(() => sendTypingRealtime(conversationId, false), 1800)
  }
  ...
  const closeChat = () => { if (conversationId != null) { dismissedConversationId = conversationId; dismissedAt = Date.now(); leaveRealtimeChat(conversationId) }; setActive(false); document.querySelector('input[placeholder="Message…"]')?.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true })) }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', flexDirection: 'column', background: '#F4F5F7', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
      <header style={{ height: 66, flexShrink: 0, display: 'flex', alignItems: 'center', gap: 11, padding: '0 14px', background: '#0B1437', color: '#fff', boxShadow: '0 1px 8px rgba(0,0,0,.12)' }}>
        ...
        <div style={{ minWidth: 0, flex: 1 }}><div style={{ fontSize: 14, fontWeight: 850, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{header}</div><div style={{ fontSize: 10, opacity: .65 }}>{group ? `${detail?.member_count || 0} members` : (typing ? 'typing…' : (realtimeConnected ? 'online' : 'offline'))}</div></div>
      ...
      <main style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '18px max(14px, calc((100vw - 760px)/2)) 14px', background: 'linear-gradient(180deg,#EEF0F3 0%,#F7F7F5 100%)' }}>
        ...
        {messages.map(message => { const mine = message.sender_id === meId; const sender = detail?.participants?.find(p => p.user_id === message.sender_id)?.display_name || 'Student'; return <div key={message.id} style={{ display: 'flex', flexDirection: 'column', alignItems: mine ? 'flex-end' : 'flex-start', marginBottom: 8 }}>
          ...
          <div style={{ maxWidth: 'min(76%, 560px)', padding: '9px 11px 7px', borderRadius: mine ? '15px 4px 15px 15px' : '4px 15px 15px 15px', background: mine ? '#0B1437' : '#fff', color: mine ? '#fff' : '#202534', boxShadow: '0 2px 7px rgba(0,0,0,.07)' }}>
            {message.attachment && <div style={{ borderRadius: 11, background: mine ? 'rgba(255,255,255,.09)' : '#F4F5F7', padding: 9, marginBottom: message.body ? 7 : 0 }}><div style={{ fontSize: 12, fontWeight: 800, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{message.attachment.original_filename}</div><div style={{ fontSize: 10, opacity: .58, marginTop: 3 }}>{(message.attachment.file_size_bytes / 1048576).toFixed(1)} MB · tap to study</div>{message.attachment.view_url && <a href={message.attachment.view_url} target="_blank" rel="noreferrer" style={{ display: 'inline-block', marginTop: 7, color: mine ? '#E4C96A' : '#8A6B20', fontSize: 10, fontWeight: 800 }}>Open document</a>}</div>}
      ...
      <footer style={{ flexShrink: 0, padding: '9px max(10px, calc((100vw - 760px)/2)) 12px', background: '#fff', borderTop: '1px solid #E4E6EA' }}>
        {attachOpen && <div style={{ display: 'flex', gap: 8, padding: '0 0 9px' }}><button type="button" onClick={() => fileRef.current?.click()} style={{ border: 0, borderRadius: 12, background: '#F1F2F4', padding: '9px 12px', fontWeight: 800, fontSize: 11, cursor: 'pointer' }}>Document</button><button type="button" onClick={() => fileRef.current?.click()} style={{ border: 0, borderRadius: 12, background: '#F1F2F4', padding: '9px 12px', fontWeight: 800, fontSize: 11, cursor: 'pointer' }}>Image</button></div>}
        ...
        <div style={{ maxWidth: 760, margin: '0 auto', display: 'flex', alignItems: 'flex-end', gap: 7 }}><button type="button" onClick={() => setAttachOpen(v => !v)} disabled={uploading} aria-label="Attach" style={{ width: 40, height: 40, flexShrink: 0, border: 0, borderRadius: 13, background: '#F0F1F3', color: '#5E6470', fontSize: 20, cursor: 'pointer' }}>+</button><button type="button" onClick={openAda} aria-label="Ask Ada" style={{ height: 40, flexShrink: 0, border: '1px solid #DCC67D', borderRadius: 13, background: '#FFF9E9', color: '#72570E', padding: '0 11px', fontWeight: 900, fontSize: 11, cursor: 'pointer' }}>@Ada</button><div style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'center', background: '#F3F4F6', border: '1px solid #E5E7EB', borderRadius: 15, padding: '0 12px' }}><input value={input} onChange={event => handleInput(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void send() } }} placeholder="Message…" disabled={sending || uploading} style={{ width: '100%', height: 42, border: 0, outline: 0, background: 'transparent', fontFamily: 'inherit', fontSize: 13 }} /></div><button type="button" onClick={() => void send()} disabled={sending || uploading || !input.trim()} aria-label="Send" style={{ width: 40, height: 40, flexShrink: 0, border: 0, borderRadius: 13, background: input.trim() ? '#C9A84C' : '#E7E8EA', color: '#0B1437', fontWeight: 900, cursor: input.trim() ? 'pointer' : 'default' }}>➤</button></div>
      </footer>
    </div>
  )
}
