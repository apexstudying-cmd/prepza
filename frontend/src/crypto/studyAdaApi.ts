...
/**
 * Sends only an explicit study-context selection to Ada.
 * This endpoint is intentionally outside the E2EE message bridge: the user
 * has chosen to disclose this exact excerpt to the AI tutor.
 */
export async function askAdaAboutSelectedStudyContext(
  ...
  if (!response.ok) {
    throw new Error(body?.error || 'Ada could not answer right now.')
  }
  if (!body || typeof body.answer !== 'string') {
    throw new Error('Ada returned an invalid response.')
  }
