...
function ChoiceCard({ selected, title, description, onClick, badge }: { selected: boolean; title: string; description?: string; onClick: () => void; badge?: string }) {
  return (
    <button onClick={onClick} style={{ width: '100%', textAlign: 'left', border: `1.5px solid ${selected ? C.gold : C.border}`, background: selected ? 'rgba(201,168,76,0.07)' : C.card, borderRadius: 15, padding: '13px 14px', marginBottom: 8, ...buttonBase }}>
...
export function PodcastGenerationScreen({ setScreen, activeDocumentId }: { setScreen: SetScreen; activeDocumentId: number | null }) {
  ...
  useEffect(() => {
    ...
    Promise.all([generationApi<{ csrf_token: string }>('/me'), generationApi<{ title: string; page_count: number | null }>(`/documents/${activeDocumentId}`)])
      .then(([me, doc]) => { setCsrf(me.csrf_token); setTitle(doc.title); setPages(doc.page_count) })
