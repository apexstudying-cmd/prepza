...
def register_study_friend_streak_routes(app, db):
    ...
    def member(conversation_id, user_id):
        return ConversationParticipant.query.filter_by(conversation_id=conversation_id, user_id=user_id, left_at=None).first()

    def peer_id(conversation, user_id):
        if not conversation or conversation.is_group:
            return None
        ids = [p.user_id for p in ConversationParticipant.query.filter_by(conversation_id=conversation.id, left_at=None).all() if p.user_id != user_id]
        return ids[0] if len(ids) == 1 else None

    def studied(user_id, day):
        seconds = db.session.query(db.func.coalesce(db.func.sum(StudyTimeLog.study_time_seconds), 0)).filter(
            StudyTimeLog.user_id == user_id, StudyTimeLog.activity_date == day).scalar() or 0
    ...
    def refresh(row):
        ...
        if row["status"] == "active":
            db.session.execute(text("""UPDATE study_friend_streak SET current_streak=:s, longest_streak=MAX(longest_streak,:s), last_shared_date=:d, updated_at=CURRENT_TIMESTAMP WHERE id=:id"""), {"s": streak, "d": today if streak else None, "id": row["id"]})
            db.session.commit()
        return streak
    ...
    def get_study_streak(conversation_id):
        ...
        row = db.session.execute(text("SELECT * FROM study_friend_streak WHERE user_a_id=:a AND user_b_id=:b"), {"a": a, "b": b}).mappings().first()
    ...
    def invite_study_streak(conversation_id):
        ...
        ensure_schema()
        a, b = pair(user_id, pid)
        if db.session.execute(text("SELECT 1 FROM study_friend_streak WHERE user_a_id=:a AND user_b_id=:b"), {"a": a, "b": b}).first():
            return jsonify({"error": "A shared streak already exists for this chat"}), 409
        count = db.session.execute(text("SELECT COUNT(*) FROM study_friend_streak WHERE status='active' AND (user_a_id=:u OR user_b_id=:u)"), {"u": user_id}).scalar() or 0
        if int(count) >= MAX_ACTIVE_SHARED_STREAKS:
            return jsonify({"error": "You already have 5 active shared streaks"}), 409
        db.session.execute(text("INSERT INTO study_friend_streak(user_a_id,user_b_id,invited_by,status) VALUES(:a,:b,:u,'pending')"), {"a": a, "b": b, "u": user_id})
        db.session.commit()
    ...
    def accept_study_streak(conversation_id):
        ...
        db.session.execute(text("UPDATE study_friend_streak SET status='active',updated_at=CURRENT_TIMESTAMP WHERE id=:id"), {"id": row["id"]})
        db.session.commit()
